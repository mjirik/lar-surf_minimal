...
\authorSection{
	...
	\anAuthor{Antonio DiCarlo}{0000-0002-7736-684X}{4}, 
	\anAuthor{V\'aclav Li\v{s}ka}{0000-0002-5226-0280}{1,5}, 
...
The input of the Linear Algebraic Representation (\textsc{lar}) Surface extraction filter (\textsc{lar-surf}) is defined as a \emph{chain}, i.e.~a vector from a linear space of chains, here subsets of voxels, represented in coordinates as a sparse binary vector. The output is produced by a linear mapping between spaces of 3- and 2-chains, given by the boundary operator $\partial_3:C_3 \to C_2$.  
...
The only data structures used by this approach are sparse arrays with one or two indices, i.e.~sparse vectors and sparse matrices. This work is based on \textsc{lar} algebraic methods and is implemented in Julia language, natively supporting parallel  computing on hybrid hardware architectures.
}
